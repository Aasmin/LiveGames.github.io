...
const bar1 = d.querySelector("#one");   // same as findElement() in cheerio
const bar2 = d.querySelector("#two");
const ball = d.querySelector("#ball");
const body = d.querySelector("body");

body.addEventListener("keydown", function (event) { // describe the keypressed event
    let bound = body.clientHeight;  //clientHeight gives body height
    console.log(bound);
    console.log(event);
    // bar1,bar2 
    // 10px
...
function moveBall() {
    ...
    if (ballTop == 0 || ballBottom == bodyheight) {
        yd = !yd    //direction ulti kr denge jb boundary touch hogi
    }
